...
use error::check;
use libc;
use mem::{Buffer, CLBuffer, Get, Put, Read, Write};
use std::cell::Ref;
use std::ffi::CString;
use std::iter::repeat;
...
impl Platform {
    fn get_devices_internal(&self, dtype: cl_device_type) -> Vec<Device> {
        unsafe {
            ...
            clGetDeviceIDs(self.id, dtype, 0, ptr::null_mut(), (&mut num_devices));

            let mut ids: Vec<cl_device_id> = repeat(0 as cl_device_id)
                .take(num_devices as usize)
                .collect();
            clGetDeviceIDs(self.id,
                           dtype,
                           ids.len() as cl_uint,
                           ids.as_mut_ptr(),
                           (&mut num_devices));
    ...
    fn profile_info(&self, name: cl_platform_info) -> String {
        unsafe {
            ...
            let status = clGetPlatformInfo(self.id, name, 0, ptr::null_mut(), &mut size);
...
lazy_static! {
    pub static ref platforms_mutex: Mutex<()> = Mutex::new(());
}
...
impl Device {
    fn profile_info(&self, name: cl_device_info) -> String {
        unsafe {
            ...
            let status = clGetDeviceInfo(self.id, name, 0, ptr::null_mut(), &mut size);
...
impl Context {
    ...
    pub fn create_program_from_source(&self, src: &str) -> Program {
        unsafe {
            ...
            let program =
                clCreateProgramWithSource(self.ctx, 1, &src.as_ptr(), ptr::null(), (&mut status));
...
impl<'r, T> KernelArg for &'r (Buffer<T> + 'r) {
    fn get_value(&self) -> (libc::size_t, *const libc::c_void) {
        unsafe { (mem::size_of::<cl_mem>() as libc::size_t, self.id_ptr() as *const libc::c_void) }
    }
}
...
impl CommandQueue {
    ...
    pub fn write_async<U: Write, T, E: EventList, B: Buffer<T>>(&self,
                                                                ctx: &Context,
                                                                mem: &B,
                                                                write: &U,
                                                                event: E)
...
///
/// Create these using
/// [`Context::create_program_from_source`](struct.Context.html#method.
/// create_program_from_source)
/// or
/// [`Context::create_program_from_binary`](struct.Context.html#method.
...
impl Program {
    ...
    pub fn build(&self, device: &Device) -> Result<String, String> {
        unsafe {
            let ret = clBuildProgram(self.prg,
                                     1,
                                     &device.id,
                                     ptr::null(),
                                     mem::transmute(ptr::null::<fn()>()),
            ...
            // Get the build log.
            let mut size = 0 as libc::size_t;
            let status = clGetProgramBuildInfo(self.prg,
                                               device.id,
...
pub fn create_kernel(program: &Program, kernel: &str) -> Kernel {
    unsafe {
        ...
        let kernel = clCreateKernel(program.prg, str.as_ptr(), (&mut errcode));
...
scalar_kernel_arg!([i32; 2]);
scalar_kernel_arg!([i64; 2]);
scalar_kernel_arg!([u32; 2]);
scalar_kernel_arg!([u64; 2]);
scalar_kernel_arg!([f32; 2]);
scalar_kernel_arg!([f64; 2]);
scalar_kernel_arg!([i32; 4]);
scalar_kernel_arg!([i64; 4]);
scalar_kernel_arg!([u32; 4]);
scalar_kernel_arg!([u64; 4]);
scalar_kernel_arg!([f32; 4]);
scalar_kernel_arg!([f64; 4]);

impl KernelArg for [f32; 3] {
    fn get_value(&self) -> (libc::size_t, *const libc::c_void) {
        (4 * mem::size_of::<f32>() as libc::size_t, (self as *const f32) as *const libc::c_void)
    }
}

impl KernelArg for [f64; 3] {
    fn get_value(&self) -> (libc::size_t, *const libc::c_void) {
        (4 * mem::size_of::<f64>() as libc::size_t, (self as *const f64) as *const libc::c_void)
    }
}

pub fn set_kernel_arg<T: KernelArg>(kernel: &Kernel, position: cl_uint, arg: &T) {
    unsafe {
...
impl Event {
    pub fn new_complete(ctx: &Context) -> Event {
        ...
        unsafe {
            clSetUserEventStatus(e, CL_COMPLETE as i32);
        }
