...
// Defines a (`Copy`, `Sync`) `struct Functions` that has extension function
// pointers as members (and methods,
// for convenience).
// Call `$ext::load()` to get an Option<$ext::Functions, String>.  (It's safe
// to call extension function pointers in other threads, right?)
...
// Whatever functions we're calling aren't necessarily thread-safe, but since
// this is a low-level
...
pub mod cl_ext_device_fission {
    ...
    pub static CL_DEVICE_PARTITION_BY_AFFINITY_DOMAIN_EXT: cl_device_partition_property_ext =
        0x4053;
